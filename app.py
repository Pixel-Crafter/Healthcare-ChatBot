...
def getDiseaseInfo(keywords):
    ...
    if results:
        ...
        return "No information found for the given keywords."
# def getDiseaseInfo(keywords):
#     results = ddg(keywords, region='wt-wt', safesearch='Off', time='y')
#     return results[0]['body']
    ...
    if request.args["message"]=="undefined":
        ...
        if currentState==4:
            ...
            if '1' in user_message or 'disease' in user_message:
                ...
                # Create a link to search YouTube for videos related to the predicted disease
                youtube_link = f'<a href="https://www.youtube.com/results?search_query={type} disease" target="_blank">Search YouTube for {type}</a>'
                response.append(youtube_link)
                
                # Create a link to search for nearby hospitals
                hospital_link = f'<a href="https://www.google.com/search?q={type} disease hospital near me" target="_blank">Search Nearby Hospitals for {type} Disease</a>'
                response.append(hospital_link)
        ...
        if currentState==5:
            if '1' in user_message or 'disease' in user_message:
                ...
                response.append(f'<a href="https://www.youtube.com/search?q={type} disease" target="_blank">youtube</a>')  
